...
As an illustration, let's focus on the **Paisley dataset**, one of the historical sources we will be exploring here (and part of the course materials). The raw data is stored as an excel spreadsheet named "paisley-data.xlsx" in the folder "data". The command `readxl()` imports this excel file into the R environment. Although `readxl` is contained in the tidyverse package, you still need to open it explicitly using `library()`.
